...
export async function POST() {
    try {
        console.log("Request received: Scheduling realistic errors for demonstration.");
        ...
        return NextResponse.json({ message: "Error generation process successfully started." });
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
        console.error("Failed to schedule errors:", errorMessage);
        return NextResponse.json({ message: "Internal server error while scheduling errors." }, { status: 500 });
    }
}
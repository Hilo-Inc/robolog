...
# 🚀 Enhanced Robolog Update System Setup
# This script sets up the enhanced update commands for an existing Robolog installation
...
echo -e "${BLUE}🚀 Setting up Enhanced Robolog Update System${NC}"
echo -e "${BLUE}===============================================${NC}"
...
# Check if robolog is already installed
if [[ ! -d "/opt/robolog" ]]; then
    echo -e "${RED}❌ Robolog not found. Please install Robolog first.${NC}"
    exit 1
fi

if ! command -v robolog &> /dev/null; then
    echo -e "${RED}❌ Robolog command not found. Please install Robolog first.${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Existing Robolog installation detected${NC}"

# Create shared directory for update scripts
echo -e "${YELLOW}📁 Creating shared directory for update scripts...${NC}"
mkdir -p /usr/local/share/robolog
chmod 755 /usr/local/share/robolog

# Copy update scripts to shared location
echo -e "${YELLOW}📋 Installing update scripts...${NC}"

# Copy update-dashboard.sh
if [[ -f "./update-dashboard.sh" ]]; then
    cp "./update-dashboard.sh" /usr/local/share/robolog/
    chmod +x /usr/local/share/robolog/update-dashboard.sh
    echo -e "${GREEN}✅ Dashboard update script installed${NC}"
else
    echo -e "${YELLOW}⚠️ update-dashboard.sh not found in current directory${NC}"
fi

# Copy update-analyzer.sh
if [[ -f "./update-analyzer.sh" ]]; then
    cp "./update-analyzer.sh" /usr/local/share/robolog/
    chmod +x /usr/local/share/robolog/update-analyzer.sh
    echo -e "${GREEN}✅ Analyzer update script installed${NC}"
else
    echo -e "${YELLOW}⚠️ update-analyzer.sh not found in current directory${NC}"
fi

# Copy update-all.sh
if [[ -f "./update-all.sh" ]]; then
    cp "./update-all.sh" /usr/local/share/robolog/
    chmod +x /usr/local/share/robolog/update-all.sh
    echo -e "${GREEN}✅ Full system update script installed${NC}"
else
    echo -e "${YELLOW}⚠️ update-all.sh not found in current directory${NC}"
fi

# Backup existing robolog command
echo -e "${YELLOW}💾 Backing up existing robolog command...${NC}"
if [[ -f "/usr/local/bin/robolog" ]]; then
    cp "/usr/local/bin/robolog" "/usr/local/bin/robolog.backup.$(date +%Y%m%d-%H%M%S)"
    echo -e "${GREEN}✅ Backup created${NC}"
fi

# Install enhanced robolog command
echo -e "${YELLOW}🔧 Installing enhanced robolog command...${NC}"
if [[ -f "./robolog-enhanced.sh" ]]; then
    cp "./robolog-enhanced.sh" /usr/local/bin/robolog
    chmod +x /usr/local/bin/robolog
    echo -e "${GREEN}✅ Enhanced robolog command installed${NC}"
else
    echo -e "${YELLOW}⚠️ Creating enhanced robolog command inline...${NC}"
    
    # Create the enhanced robolog command inline
...
SCRIPT_DIR="/usr/local/share/robolog"
...
EOF
    ...
    chmod +x /usr/local/bin/robolog
    echo -e "${GREEN}✅ Enhanced robolog command created${NC}"
fi

# Test the new commands
echo -e "${YELLOW}🧪 Testing enhanced commands...${NC}"

# Check if the new help shows update commands
if /usr/local/bin/robolog | grep -q "update analyzer"; then
    echo -e "${GREEN}✅ Enhanced update commands detected${NC}"
else
    echo -e "${RED}❌ Enhanced update commands not working properly${NC}"
    exit 1
fi

echo ""
echo -e "${GREEN}🎉 Enhanced Robolog Update System installed successfully!${NC}"
echo ""
echo -e "${BLUE}📋 Available update commands:${NC}"
echo -e "${YELLOW}  sudo robolog update analyzer    ${NC}- Update only the analyzer component"
echo -e "${YELLOW}  sudo robolog update dashboard   ${NC}- Update only the dashboard component"
echo -e "${YELLOW}  sudo robolog update all         ${NC}- Update all components and system packages"
echo ""
echo -e "${BLUE}🔍 To see all available commands:${NC}"
echo -e "${YELLOW}  robolog${NC}"
echo ""
echo -e "${BLUE}💡 Backup information:${NC}"
echo -e "Original robolog command backed up to:"
echo -e "${YELLOW}/usr/local/bin/robolog.backup.*${NC}"
echo ""
echo -e "${BLUE}📁 Update scripts installed in:${NC}"
echo -e "${YELLOW}/usr/local/share/robolog/${NC}"
echo ""